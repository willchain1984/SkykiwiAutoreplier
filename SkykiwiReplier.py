...
class MyFrame(wx.Frame):
    ...
    def MyFrameOnClose(self, event):
        ...
        if not self.IsFirstLogin:
            context = {
                "credential": {
                    "username": self.m_user.Value,
            ...
            if self.IsLogin:
                task_name = "TASK-" + self.credential["username"]
                try:
                    with open(task_name, "wb") as f:
                        pickle.dump(self.tasks, f)
                        logging.info(
                            f"Successed to save the schedule when exit. User:{self.credential['username']}"
                        )
                except Exception:
                    logging.warning(
                        f"Failed to save the schedule when exit. User:{self.credential['username']}"
                    )
        # self.Destroy()
        self.Hide()
        event.Skip()
    ...
    def _async_login(self):
        if not self.IsLogin:
            ...
            if ret == True:
                task_name = "TASK-" + user
                if task_name in listdir():
                    try:
                        with open(task_name, "rb") as f:
                            self.tasks = pickle.load(f)
                    except Exception:
                        logging.warning("Failed to obtain the saved schedule.")
                items = []
            ...
            if self.discuz.logout():
                ...
                try:
                    with open(task_name, "wb") as f:
                        pickle.dump(self.tasks, f)
                    logging.info(
                        f"Successed to save the schedule when user logout. User:{self.credential['username']}"
                    )
                except Exception:
                    logging.warning(
                        f"Failed to save the schedule when user logout. User:{self.credential['username']}"
                    )
                self.tasks.clear()
                self.m_lbox_tidlist.Clear()
